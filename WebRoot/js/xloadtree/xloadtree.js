...
function WebFXLoadTree(sText, sXmlSrc, sAction, sBehavior, sIcon, sopen_icon) {
	// call super
	this.WebFXTree = WebFXTree;
	this.WebFXTree(sText, sAction, sBehavior, sIcon, sopen_icon);
	...
	if (this.open)
		...
		this._loadingItem = new WebFXTreeItem({text:webFXTreeConfig.loadingText});
...
//function WebFXLoadTreeItem(sText, sXmlSrc, sAction, eParent, sIcon, sopen_icon) {
function WebFXLoadTreeItem(loadTreeInfo) {
	// call super
	this.WebFXTreeItem = WebFXTreeItem;
	//this.WebFXTreeItem(sText, sAction, eParent, sIcon, sopen_icon);
	this.WebFXTreeItem(loadTreeInfo);
	// setup default property values
	var sXmlSrc = loadTreeInfo.xmlSrc;
...
WebFXLoadTreeItem.prototype.reload = function () {
	...
	if (this.loaded) {
		...
		*/
		this.childNodes  = [];
		document.getElementById(this.id+'-cont').innerHTML = '';
		...
		if (xmlHttp.readyState == 4) {
			...
			try{
			   ...
			   try{
			     ...
			     oXmlDoc=parser.parseFromString(xml,"text/xml");
...
function _xmlTreeToJsTree(oNode) {
	...
	var open_icon = oNode.getAttribute("open_icon");
	var src = oNode.getAttribute("src");
	var target = oNode.getAttribute("target");
	//add code
	var data = oNode.getAttribute("data");
	//add code
	var treeInfo = {text:text,action:action,parent:parent,icon:icon,open_icon:open_icon,xmlSrc:src,data:data};
	
	// create jsNode
	var jsNode;
	if (src != null && src != "")
		//jsNode = new WebFXLoadTreeItem(text, src, action, parent, icon, open_icon);
		jsNode = new WebFXLoadTreeItem(treeInfo);
	else
		//jsNode = new WebFXTreeItem(text, action, parent, icon, open_icon);
		jsNode = new WebFXTreeItem(treeInfo);
...
function _xmlFileLoaded(oXmlDoc, jsParentNode) {
	...
	if( oXmlDoc == null || oXmlDoc.documentElement == null) {
		...
		if(l>500 && threadExist && webFXTreeConfig.useThread){
			for (var i = 0; i < l; i++) {
				if (cs[i].tagName == "tree") {
					bAnyChildren = true;
					bIndent = true;
			...
			var pageTotal = l;
			var pageSize = 100;
			var pageCount = Math.floor(pageTotal/pageSize)+(pageTotal%pageSize==0?0:1);
			for(var i =0;i<pageCount-1;i++){
				start = i * pageSize;
				end = start + pageSize;
				Concurrent.Thread.create(function(jsParentNode,cs,start,end){
					for (var i = start; i < end; i++) {
						if (cs[i].tagName == "tree") {
							jsParentNode.add( _xmlTreeToJsTree(cs[i]), true);
						}
					}
				},jsParentNode,cs,start,end);
			}
